...
type elementType int // most tests use this as the set's element type
...
func TestZero(t *testing.T) {
	var s rangeset.Set[elementType]
...
func TestEmptyAndUniversalComplement(t *testing.T) {
	empty := rangeset.Make[elementType]()
	complement := rangeset.Complement(empty)
	u := rangeset.Universal[elementType]()
	Assertf(t, rangeset.Equal(u, complement), "%24s: expected complement of empty set to be %v got %v\n",
		"TestEmptyUniversal", u, complement)

	complement = rangeset.Complement(u)
	Assertf(t, rangeset.Equal(empty, complement), "%24s: expected complement of U to be %v got %v\n",
